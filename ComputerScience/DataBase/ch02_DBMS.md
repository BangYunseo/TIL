...
| 객체관계 DBMS | 객체지향 DBMS + 관계 DBMS  | 관계 DBMS 제품들이 객체지향 기능을 지원하며 객체관계 DBMS로 분류 가능(ex : 오라클(Oracle)) |
...
|    종류     | 설명                                                                                                                                                                                                              | 예시                                                                                                                   |
| :---------: | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------- |
| NoSQL DBMS  | 1. 비정형 데이터를 처리에 적합<br>뛰어난 확장성<br>2. 안정성과 일관성 유지를 위한 복잡한 기능 포기<br>3. 데이터 구조를 미리 결정하지 않는 유연성<br>4. 여러 대의 컴퓨터에 데이터 분산·저장·처리하는 환경에서 사용 | 몽고디비(MongoDB) · H베이스(HBase) · 카산드라(Cassandra) · 레디스(Redis) · 네오포제이(Neo4j) · 오리엔트DB(OrientDB) 등 |
...
|   성능   | 설명                                                                                                                                                            |
| :------: | :-------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|  유연성  | 1. 유연한 스키마를 제공<br>2. 신속·반복적 개발 가능<br>3. 유연한 데이터 모델을 가져 반정형·비정형 데이터에 이상적                                               |
|  확장성  | 1. 고가의 강력한 서버를 추가<br>2. 분산형 하드웨어 클러스터로 확장하도록 설계<br>3. 일부 클라우드 제공자들은 완전관리형 서비스로서 분산 운영 작업을 숨겨서 처리 |
|  고성능  | 1. 특정 데이터 모델·액세스 패턴에 최적화<br>2. 관계형 데이터베이스로 유사 기능을 충족할 때보다 뛰어난 성능                                                      |
| 고기능성 | 1. 각 데이터 모델에 맞춰 특별히 구축된 뛰어난 기능의 API·데이터 유형 제공                                                                                       |
